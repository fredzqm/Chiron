...
def save_model():
    ...
    toPath = os.path.join(FLAGS.log_dir, FLAGS.model_name, 'model')
...
def inference(x, seq_length, training):
    ...
    logits = rnn_layers_one_direction(cnn_feature, seq_length, training, class_n=4 ** FLAGS.k_mer + 1)
...
if __name__ == "__main__":
    class Flags():
        def __init__(self):
            self.home_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), os.path.pardir)
            self.data_dir = os.path.join(self.home_dir, 'input')
            self.cache_dir = os.path.join(self.home_dir, 'train', 'cache')
            self.log_dir = os.path.join(self.home_dir, 'train', 'log')
